...
{
	size_t length{ dateOfRegistration.size() };
	length = (length < 11 ? length : 10);
	dateOfRegistration.copy(date, length);
	date[length] = '\0';
}
...
void Patient::setFamilyDisease(const string& fDisease)
{
	...
	fDisease.copy(familyDisease, length);
	familyDisease[length] = '\0'; // append null character to past Medical History
}

// get patients's family disease
string Patient::getFamilyDisease() const
{
